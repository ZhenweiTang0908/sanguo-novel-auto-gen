...
function ChapterContent({ chapter, buildBackUrl, buildNavUrl }: { chapter: ChapterDetail; buildBackUrl: () => string; buildNavUrl: (id: number) => string }) {
        ...
        <p key={index} className="text-gray-700 leading-relaxed">
          {parts.map((part, i) => {
            if (part.startsWith('**') && part.endsWith('**')) {
              return <strong key={i} className="text-orange-600 font-semibold">{part.slice(2, -2)}</strong>;
            }
            return part;
          })}
        </p>
    ...
    <div className="min-h-screen relative z-10">
      ...
      <main className="max-w-4xl mx-auto px-6 py-10">
        ...
        <nav className="mt-8 flex items-center justify-between gap-4 animate-fade-in-up delay-200" style={{ opacity: 0 }}>
          {chapter.hasPrev ? (
            <Link href={buildNavUrl(chapter.id - 1)} className="nav-btn flex-1">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function ChapterReaderInner({ chapterId }: ChapterReaderProps) {
  ...
  const buildNavUrl = useCallback((targetId: number) => {
  ...
  if (error || !chapter) {
    return <ErrorState error={error || '章节不存在'} backUrl={buildBackUrl()} />;
  }

  return <ChapterContent chapter={chapter} buildBackUrl={buildBackUrl} buildNavUrl={buildNavUrl} />;
}

export default function ChapterReader({ chapterId }: ChapterReaderProps) {
  return (
    <Suspense fallback={<LoadingState />}>
      <ChapterReaderInner chapterId={chapterId} />
    </Suspense>
  );
}